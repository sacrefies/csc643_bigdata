...
film_inventory_w_category = JOIN film BY film_id, film_category BY film_id,
                                 inventory BY film_id USING 'replicated';
film_full_info = JOIN film_inventory_w_category BY film_category::category_id,
                      category BY category_id USING 'replicated';
film_inventory_rented = JOIN film_full_info BY inventory::inventory_id,
                             rental BY inventory_id USING 'replicated';
customers_who_rented_films = JOIN film_inventory_rented BY rental::customer_id,
